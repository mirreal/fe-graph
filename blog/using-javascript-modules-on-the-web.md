...
JS 模块（也称为“ES 模块”或“ECMAScript模块”）是 ES6 中一项非常重要的语言特性。在此以前，你可能使用过用户级别的 JavaScript 模块系统，比如[在 Node.js 中的 CommonJS](https://nodejs.org/docs/latest-v10.x/api/modules.html)，或者是 [AMD](https://github.com/amdjs/amdjs-api/blob/master/AMD.md)，或者其他别的实现。所有的模块系统都包含一个共同点：允许导入和导出内容。
...
* 模块中不支持 HTML 注释语法。
  ...
  const x = 42; // TODO: Rename x to y.
  ```
* 模块拥有一个顶级词法作用域。也就是说，如果在一个模块中运行 `var foo = 42;` *不会*创建一个名为 `foo`，可以在浏览器通过`window.foo` 访问的全局变量。
...
另一个区别与 `async` 属性有关，它可以让脚本下载而不阻止 HTML 解析器（就像 `defer`），但会立即执行脚本，不保证顺序，并且不需要等待 HTML 解析完成。`async` 属性不适用于内联经典脚本，但仍适用于内联 `<script type="module">`。
...
与静态 `import` 不同，动态 `import()` 能在常规脚本中使用，可以从这里开始在现有代码库中逐步使用 JS 模块。有关更多相关信息，请参阅[我们关于动态 import 的文章](https://developers.google.com/web/updates/2017/11/dynamic-import)。
...
借助 JS 模块，可以在不使用打包工具（如 webpack，Rollup 或 Parcel）的情况下开发网站。在以下情况下，直接使用原生 JS 模块会更具优势：
...
在 web 开发中，使用非打包模块可能会降低初始加载性能（冷缓存），但实际上可以提高后续访问（热缓存）的加载性能。对于一个没有使用代码拆分的 200 KB 的代码库，在更改一个细粒度的模块时，从服务器单独获取这个模块要比重新获取整个 bundle 更好。

如果你更关心使用热缓存的访问者的体验，而不是首次访问的性能，并且网站的细粒度模块少于几百个，那么可以尝试使用非打包模块。需要做的是评估冷缓存和热缓存的加载性能，然后做出以数据驱动的决定。

浏览器工程师正在努力改进即开即用的模块性能。随着时间的推移，预计在更多情况下，使用非打包模块将会成为可行。
...
在这种情况下（没有使用构建时打包步骤），浏览器仍然需要下载，解析和编译整个 `./util.mjs` 模块，即使需要的只是其中的一个导出。这很浪费性能。

如果 `pluck` 不和 `drop` 和 `zip` 共享任何代码，更好的方式是把它移动到自己的细粒度模块，比如 `./pluck.mjs`：
...
**注意：**可以在这里使用 `default` 导出而不是命名导出，取决于个人喜好。

这不仅可以让源代码更加简单，而且还可以减少打包工具消除死代码的需求。如果源代码树中的某个模块未被使用，那么它永远不会被导入，所以浏览器永远不会下载它。并且，浏览器可以单独为该模块进行[代码缓存](https://v8project.blogspot.com/2018/04/improved-code-caching.html)。
...
所以应该继续坚持推进使用 HTTP/2，但 HTTP/2 服务器推送并不是银弹。

## JS 模块在 Web 的使用情况

在 web 上使用 JS 模块正在逐渐变得流行起来，但[在我们的使用情况统计](https://www.chromestatus.com/metrics/feature/timeline/popularity/2062) 显示目前仅有 0.08％ 的页面在使用 `<script type="module">`，这份数据不包含像是动态 `import()` 或 [worklets](https://drafts.css-houdini.org/worklets/) 的使用 JS 模块的方式。

## JS 模块的下一步

Chrome 团队正在尝试各种方式改善 JS 模块的开发体验，现在来讨论其中的一些点。

### 更快和确定的模块解析算法

我们提出了改进模块解析算法的提案，以解决目前在速度和确定性上的不足。新算法既存在于 [HTML 规范中](https://github.com/whatwg/html/pull/2991)，也存在于 [ECMAScript 规范中](https://github.com/tc39/ecma262/pull/1006)，并在 [Chrome 63 中](http://crbug.com/763597)实现。预计这种改进很快就会在更多的浏览器中得以实现。

新算法效率更高，速度更快。旧算法的复杂度是依赖图大小的二次方，即 O(n²)，Chrome 的实现也是如此。新算法的复杂度是线性的，即 O(n)。

而且，新算法会以更确定的方式报告解析错误。给定一个包含多个错误的依赖图，旧算法在不同运行情况下可能会报告不同的错误，因为它们都会导致解析失败，这使调试非常困难，而新算法每次都报告相同的错误。

### Worklets 和 web workers

Chrome 现在实现了 [wokelets](https://drafts.css-houdini.org/worklets/)，它允许 web 开发人员自定义浏览器底层级别的硬编码逻辑。通过使用 worklets，Web 开发人员可以将 JS 模块提供给渲染管道或音频处理管道（未来可能还会有更多管道）。
...
Chrome 66 支持 [`AudioWorklet`](https://developers.google.com/web/updates/2017/12/audio-worklet)，可使用自定义代码控制音频处理。同时加入 [OriginTrial for`AnimationWorklet`](https://groups.google.com/a/chromium.org/d/msg/blink-dev/AZ-PYPMS7EA/DEqbe2u5BQAJ)，可以用来创建 scroll-linked 效果以及其他高性能程序动画。

最后，[`LayoutWorklet`](https://drafts.css-houdini.org/css-layout-api/)（也就是 CSS Layout API）在 Chrome 67 中实现。

我们正在[实现](https://bugs.chromium.org/p/chromium/issues/detail?id=680046)为 Chrome 中的专用 web worker 提供对 JS 模块的支持。可以在 `chrome://flags/#enable-experimental-web-platform-features` 启用后尝试此功能 。

```js
const worker = new Worker('worker.mjs', { type: 'module' });
```

shared worker 和 service worker 也即将支持 JS 模块：
...
在 Node.js/npm 中，通常通过“包名称”来导入 JS 模块。比如：
...
目前，[根据 HTML 规范](https://html.spec.whatwg.org/multipage/webappapis.html#resolve-a-module-specifier)，这种“纯导入标识符”会引发异常。[我们提出包名称映射的提案](https://github.com/domenic/package-name-maps)允许此类代码在 Web 上运行，包括生产环境。包名称映射是一个 JSON 文件，用来帮助浏览器将纯导入标识符转换为完整的 URL。

目前包名称映射仍处于提案阶段，而且还没有完整的规范。

### web packing：native bundles

Chrome 浏览器团队目前正在探索[一种原生的 web 打包格式](https://github.com/WICG/webpackage)作为分发网络应用的新方式。其核心功能是：

[签名的 HTTP Exchange](https://wicg.github.io/webpackage/draft-yasskin-http-origin-signed-responses.html)允许浏览器相信单个 HTTP 请求/响应对声称的来源生成; [打包的 HTTP Exchange](https://wicg.github.io/webpackage/draft-yasskin-dispatch-bundled-exchanges.html)，即一组交换信息，每个交换信息都可以是签名或未签名的，其中包含一些元数据描述如何将包作为一个整体解析。

这样的组合打包格式将使 *多个同源的资源* 被 *安全地嵌入* *单个* HTTP `GET` 响应。

现有的打包工具（如 webpack，Rollup 或 Parcel）将生成单个 JavaScript 包，其中原始模块的语义丢失。使用原生打包，浏览器可以将资源分解回原来的形式。简而言之，可以将 Bundled HTTP Exchange 想象成可通过内容目录以任意顺序访问的资源束，并且所包含的资源可以根据其重要性进行有效存储和标记，同时保持独立文件的概念。正因为如此，原生打包可以改善调试体验，在 DevTools 中查看资源时，浏览器可以精确定位原始模块，而不依赖复杂的 source map。

由于原生打包格式的透明度，可以轻松地完成一些优化。比如，当浏览器已经缓存了包的一部分时，可以通知 Web 服务器，然后只下载缺失的部分。

Chrome 已经支持提案（[`SignedExchanges`](https://wicg.github.io/webpackage/draft-yasskin-http-origin-signed-responses.html)）的一部分，但打包格式本身以及其对高度模块化的应用仍处于探索阶段。

### Layered API

发布新特性和 Web API 会导致持续的维护和运行时成本，每个新特性都会污染浏览器名称空间，增加启动成本，并且可能在整个代码库中引入 bug。[Layered API](https://github.com/drufball/layered-apis) 希望以更具扩展性的方式实现和发布一些 Web 浏览器的高级 API。JS 模块是 Layered API 中的关键技术：

* 由于模块是明确导入的，因此通过模块暴露 Layered API 可以确保开发人员只关心他们使用到的部分。
* 由于模块加载是可配置的，Layered API 可以实现一种内置机制用于在不支持 Layered API 的浏览器中自动加载 polyfills。

模块和 Layered API 如何协同工作的细节[仍在制定中](https://github.com/drufball/layered-apis/issues)，但目前的提案看起来像这样：
